...
-- Law: either/empty
-- To be able to keep the short circuit of the operator, `either` should stop
-- at the first `empty` challenge
-- ...🤮
∀ (c :: Challenge).
  not (isReward c) =>
    either empty c = empty

-- `both` will "complete" only when both challenges are completed
-- (ie. they will reduce to an `empty` challenge)

-- `either` will "complete" when at least one of the challeges are
-- completed (ie. they will reduce to an `empty` challenge)
...
∀ (i :: Maybe Input) (c1 :: Challenge) (c2 :: Challenge).
  step i (either c1 c2) =
    ...
    # either <$> (r1, c1') = (r1, either c1' :: Challenge -> Challenge)
    # (r1, either c1' :: Challenge -> Challenge) <*> (r2, c2')
...
-- `either` challenge with `c1'` and `c2'`, that is until one of the challenge
-- will be `empty`
#+END_EXAMPLE
...
*Exercise:* Reduce ~either (reward r1) empty~ to its simplest form

#+BEGIN_EXAMPLE
either (reward r1) empty
either empty empty
empty
...
-- challenges will be reduced to `empty`

-- The first challenge will be completed (ie. reduced to `empty`) after one
-- hour, assuming the second challenge will not be completed in the meantime,
-- then the `either` challenge will be completed taking away the possibility
-- to complete the second challenge
#+END_EXAMPLE
...
#+BEGIN_SRC haskell :eval never
-- Until now we have a constructor of a Challenge given a Clue and another
-- Challenge aka a way to attach a Clue to a Challenge
clue :: Clue -> Challenge -> Challenge

-- Constructor of a clue
hint :: String -> Clue

-- Clue can be nested? Why not, with this we have a semigroup given the
-- associative law below, and a monoid with the identity element below
sub :: Clue -> Clue -> Clue
...
-- Monoid endomorphism = when a monoid homomorphism form a monoid
#+END_EXAMPLE
